...
% Initialize the node with a factor table if it is a factor node and a
% factor table has not been provided already, and the factor node does not
% have any parents
if obj.type == NodeType.factor && ~cp && isempty(obj.parents)
    
    % Create a factor with random probability distribution
    fac = obj.factor.makeFactor({obj.name}, {obj.values}, {'probability'});
    fac.tableNames = {obj.name, 'probability'};
    fac.tableValues = obj.values;
